...
module.exports = (grunt) => {
  ...
  grunt.initConfig({
    ...
    concat: {
      license: {
        ...
        files: {
          'dist/ghostban-tinymce-plugin/plugin.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ],
          'dist/ghostban-tinymce-plugin/plugin.min.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ]
        }
    ...
    copy: {
      css: {
        files: [
          { src: [ 'CHANGELOG.txt', 'LICENSE.txt' ], dest: 'dist/ghostban-tinymce-plugin', expand: true }
        ]
      }
    },
  ...
  grunt.registerTask('version', 'Creates a version file', () => {
    grunt.file.write('dist/ghostban-tinymce-plugin/version.txt', BUILD_VERSION);
  });
